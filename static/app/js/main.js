...
// pendingFiles: [{ file, relativePath }]
let pendingFiles = [];
...
fileInput?.addEventListener('change', () => {
  ...
  const items = Array.from(fileInput.files).map(f => ({ file: f, relativePath: f.name }));
...
// ── Drag & Drop รองรับทั้งไฟล์และโฟลเดอร์ ──────────────────────────────────
function handleDrop(dataTransfer) {
  if (!dataTransfer) return;
  const items = dataTransfer.items ? Array.from(dataTransfer.items) : null;

  // ถ้า browser รองรับ webkitGetAsEntry → อ่าน folder tree
  if (items && items[0]?.webkitGetAsEntry) {
    ...
    function readEntry(entry, path) {
      if (entry.isFile) {
        pending++;
        entry.file(f => {
          collected.push({ file: f, relativePath: path + f.name });
          pending--;
          if (pending === 0) setPendingFiles(collected);
        });
        ...
        function readAll() {
          ...
          reader.readEntries(entries => {
            ...
            if (entries.length === 100) readAll(); // Chrome limits 100 per batch
            if (pending === 0 && collected.length) setPendingFiles(collected);
          });
        }
        readAll();
      }
    ...
    // fallback
    const items = Array.from(dataTransfer.files).map(f => ({ file: f, relativePath: f.name }));
    setPendingFiles(items);
  }
}

function setPendingFiles(items) {
  pendingFiles = items;
  renderPreview();
  syncToForm();
}
...
function renderPreview() {
  ...
  const folders = {};
  let rootFiles = 0;
  pendingFiles.forEach(({ file, relativePath }) => {
  ...
  Object.entries(folders).forEach(([folder, count]) => {
    ...
    div.innerHTML = `
      <svg viewBox="0 0 24 24" fill="#f59e0b" width="14" height="14"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg>
      <span class="upload-file-name">${escHtml(folder)}/</span>
      <span class="upload-file-size">${count} ไฟล์</span>`;
    filePreview.appendChild(div);
  });

  // แสดงไฟล์ root
  pendingFiles.filter(({ relativePath }) => !relativePath.includes('/')).forEach(({ file }) => {
    const div = document.createElement('div');
    div.className = 'upload-file-item';
    div.innerHTML = `<span class="upload-file-name">${escHtml(file.name)}</span><span class="upload-file-size">${humanSize(file.size)}</span>`;
    filePreview.appendChild(div);
  });

  if (uploadSubmit) uploadSubmit.disabled = pendingFiles.length === 0;
}

function syncToForm() {
  // ใส่ไฟล์จริงใน file input
  const dt = new DataTransfer();
  pendingFiles.forEach(({ file }) => dt.items.add(file));
  if (fileInput) fileInput.files = dt.files;

  // ใส่ relative paths เป็น hidden inputs
  const container = document.getElementById('upload-relative-paths');
  if (container) {
    container.innerHTML = '';
    pendingFiles.forEach(({ relativePath }) => {
      const inp = document.createElement('input');
      inp.type = 'hidden';
      inp.name = 'relative_paths';
      inp.value = relativePath;
